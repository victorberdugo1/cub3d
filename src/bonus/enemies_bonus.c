...
/*   Updated: 2025/04/18 15:44:21 by victor           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_enemy_direction(t_app *app, t_enemy *e, double delta_time)
{
	...
	target_angle = atan2(app->camera.pos.y - e->pos_y,
	...
	angle_diff = target_angle - e->facing_angle;
	angle_diff = fmod(angle_diff + M_PI, 2 * M_PI) - M_PI;
	max_rotation_speed = 1.0 * delta_time;
	...
	player_angle = atan2(app->camera.pos.y - e->pos_y,
			app->camera.pos.x - e->pos_x);
	relative_angle = player_angle - e->facing_angle;
